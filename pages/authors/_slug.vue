...
export default {
  async asyncData({ params, route, app, error }) {
    ...
    if (!params.slug) {
      return error({ statusCode: 404, message: 'Author not found' })
    }
    try {
      ...
      const [pageDataRes, allArticlesRes] = await Promise.all([
        ...
        app.$storyapi.get('cdn/stories/', {
          ...
          starts_with: 'blogs/',
          per_page: 100,
          resolve_relations: 'blog.categories,blog.author',
        }),
      ])
      return {
  ...
  head() {
    const authorName = this.getAuthoData?.content?.Name || 'Author'
    const authorDesc = `Explore blog posts by ${authorName} at Vodworks.` // ||  this.getAuthoData?.content?.Description
    return {
      title: `${authorName} – Blog Author at Vodworks`,
      meta: [
        {
          hid: 'description',
          name: 'description',
        ...
        },
        {
          hid: 'og:title',
          property: 'og:title',
          content: `${authorName} – Blog Author at Vodworks`,
        },
        {
          hid: 'og:description',
  ...
  computed: {
    ...
    //   const currentAuthor = this.pageData.story
    //   const authorUuid = currentAuthor.uuid
    //   const authorSlug = currentAuthor.full_slug?.split('/').pop() // e.g. "jaffer-kazim"

    //   return this.allArticles.stories
    //     .filter(blog => {
    //       const author = blog.content.author

    //       // ✅ Case 1: Author resolved with uuid
    //       if (author?.uuid && author.uuid === authorUuid) return true

    //       // ✅ Case 2: Match via slug (team/ or authors/)
    //       if (author?.full_slug?.endsWith(authorSlug)) return true

    //       // ✅ Case 3: Author returned as raw string id
    //       if (typeof author === 'string' && author === authorUuid) return true

    //       return false
    //     })
    //     .sort((a, b) => new Date(b.content.published_date) - new Date(a.content.published_date))
    // },

    getFilteredBlogPosts() {
      // current author's uuid (from the page you’re on)
      const authorUuid = this.pageData.story.uuid
      return this.allArticles.stories
        .filter(blog => {
          return blog.content.author?.uuid === authorUuid
        })
        .sort(
          (a, b) =>
